...
//static BYTE trumpet_shiftY[] = {-8,-7,-6,-4,-2,0,2,4};
...
static UWORD palette2[] = {0X112,0X222,0X234,0X357,0X589,0X7ab,0X9dd,0Xcff};
...
void trumpetplayer(void){
    ...
    // trumpets
    UBYTE trumplePlayer_height = 0;
    UBYTE trumpetDrawStep = 0;
    ...
    UBYTE *sdata2;
    int rvalue;
    //struct ShrinklerHeader *shrinklerHeader;
    //struct ShrinklerFile *shrinklerFile;
    ...
    printf("TrumpetPlayer ready\n");
    printf("Free Chip memory: %d\n", AvailMem(MEMF_CHIP));
    printf("Free Fast memory: %d\n", AvailMem(MEMF_FAST));

    //palette_set(palette1,32);
    WaitTOF();

    // spotlight1.planes.min2: generated from spotlight1.planes using shrinkler with option -b
    //dataIndex = loadFile("data/spotlight1.planes.min3", 1);
    //printf("DataIndex: %d\n", dataIndex);

    //shrinklerLoad("data/spotlight1.planes.min", MEMF_CHIP, 0);
    //shrinklerFile = (struct ShrinklerFile *) getAssetData(dataIndex);
    //sdata = (UBYTE *) getAssetData(dataIndex);
    //shrinklerHeader = shrinklerFile->header;
    //printf("Shrinkler Header Size: %d\n", sizeof(struct ShrinklerHeader));
    
    //uncompressedSize = (shrinklerHeader->unCompSize1 << 24) + (shrinklerHeader->unCompSize2 << 16) + (shrinklerHeader->unCompSize3 << 8) + shrinklerHeader->unCompSize4;
    
    //uncompressedSize = shrinklerFile->header.unCompSize;
    //printf("Uncompressed Size: %d\n", uncompressedSize);

    //sdata2 = AllocMem(30720, MEMF_CHIP);
    //shrinklerDecompress(sdata, sdata2, 0,0,0,0);
    //shrinklerDecompress(shrinklerFile->compressedData, sdata2, 0,0,0,0);

    //rvalue = shrinklerLoad(imgfile, MEMF_CHIP, 0, 0, 0);
    //sdata2 = rvalue;
    ...
    //screen_drawAsset_simple(backgroundAsset, 0 ,0);
    //screen_drawAsset2(trumpetAsset, 112 ,76);

    //screen_drawAsset2(backgroundAsset, 0, 0);
	...
	while(loop){
        ...
        OwnBlitter();
            ...
            if (spotlightActive){
                ...
                if (spotlightFill){
                    ...
                    bitplane_fillArea_simple(spotLightPlane,0, 1, 300, 255, 0, 0);
                }
            }
		...
		if(isMouseDown()) loop = FALSE;	// quit when lmb pressed
